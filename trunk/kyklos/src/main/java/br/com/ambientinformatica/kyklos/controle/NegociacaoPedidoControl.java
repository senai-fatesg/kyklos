...
public class NegociacaoPedidoControl implements Serializable {
	...
	public void selecionarProduto(SelectEvent event) throws PedidoException {
		try {
			...
			listaEstoque = EstoqueProdutoDto
					.converterEstoqueProduto(estoqueProdutoDao.listarPorEmpresa(
							usuarioLogadoControl.getEmpresaUsuario().getEmpresa(),
							produto));
	...
	public List<Produto> completeProduto(String codigoDescricao) {
		return produtoDao.listarProdutos(usuarioLogadoControl.getEmpresaUsuario()
				.getEmpresa(), codigoDescricao);
	}
	...
	public void listarPedidos(ActionEvent event) {
		pedidosFiltro = pedidoDao.listarPedidoPorAtributo(usuarioLogadoControl
				.getEmpresaUsuario().getEmpresa(), pedido.getNumero(), dataInicial,
				dataFinal, cliente, vendedor, statusPedidoSelecionados);
